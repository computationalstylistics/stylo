...
# so far, this is the original stylo() function plugged into a wrapper;
# see the new stylo() wrapper function in the stylo.R file





run_stylo = function(gui = FALSE,
             frequencies = NULL,
             parsed.corpus = NULL,
             features = NULL,
             path = NULL,
...
# to use it without explicit references to `path = NULL` and the other vars
#
#run_stylo = function(...) {


	#config = list(...)
	# manually assign parameters set by the user
	#config$mfw.max = mfw.max
	#config$mfw.min = mfw.min

	# But so far, using the original `variables = list(...)`, see line 72
	variables = list(...)

	# switch GUI off forever -- any reference to GUI will be deleted;
	# the current engine function shouldn't even know that any GUI exist
	# it will inherit the full config from the wrapper function oppose()
	gui = FALSE
...
# (it absorbs the arguments passed from command-line)
#variables = stylo.default.settings(...)

# switching this ^ off, because the variables will already be set





# the whole GUI call should be deleted from here after testing
